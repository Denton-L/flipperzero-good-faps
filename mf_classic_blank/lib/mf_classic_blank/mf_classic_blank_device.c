...
bool mf_classic_blank_device_block_writable(const MfClassicData* data, uint8_t block_num, MfClassicKeyType* key_type_out, MfClassicAction* action_out) {
    uint8_t sector_num = mf_classic_get_sector_by_block(block_num);
    for (MfClassicKeyType key_type = 0; key_type < COUNT_OF(mf_classic_blank_device_key_type_to_write); ++key_type) {
...
static MfClassicDeviceError mf_classic_blank_device_mifare_classic_check(NfcDevice* device, const MfClassicData** data_out) {
